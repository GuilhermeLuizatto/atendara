...
import { RULE_CATEGORY_LABELS } from "./labels-config.js";
...
export function interpretRuleText(input) {
    ...
    const matcher = CATEGORY_MATCHERS.map((candidate) => ({
        ...candidate,
        specificity: Math.max(0, ...candidate.terms
    ...
    if (!matcher) {
        warnings.push("Não identificamos o assunto da regra. Revise a categoria antes de ativar.");
    }
    const category = matcher?.category ?? "GENERAL";
    ...
    if (durationMatch)
        payload.durationMinutes = Number(durationMatch[1]);
    const actions = [
    ...
    if (conditions.length === 1 && text.includes(" se ")) {
        warnings.push("Há uma condição no texto que não foi reconhecida. Revise antes de ativar.");
        confidence -= 0.15;
    }
    const name = matcher
