...
import type { Client, OutboundChannel } from "@/types";
...
export interface NormalizedPhone {
  /** Numero em E.164, pronto para o provedor. */
  e164: string;
  /**
   * `true` quando o codigo do pais foi assumido, e nao informado. Antes de
   * qualquer integracao real este caso precisa de confirmacao humana: os mesmos
   * digitos podem pertencer a outro pais.
   */
...
export interface ChannelContact {
  destination: string;
  /** Forma reduzida guardada no registro de entrega. */
  hint: string;
  countryAssumed: boolean;
}
