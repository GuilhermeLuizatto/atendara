...
function matches(destination: string, group: readonly string[]): boolean {
  return group.some((value) => value === destination.toLowerCase());
}

function decide(request: SendRequest): SendResult {
  ...
  if (
    matches(destination, SIMULATED_DESTINATIONS.recoversOnRetry) &&
    request.attempt === 1
  ) {
    return {
      outcome: "TEMPORARY_FAILURE",
...
export function createSimulatedProvider(): NotificationProvider {
  return {
    id: "SIMULATED",
    simulated: true,
