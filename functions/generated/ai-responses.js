...
export function composeResponse(intent, { profession, organization, rule }) {
    ...
    const payload = rule.actions.find((action) => ["ALLOW_TOPIC", "PROVIDE_INFO", "AUTO_RESPONSE"].includes(action.type))?.payload;
    const price = typeof payload?.priceInCents === "number"
        ? payload.priceInCents
        : profession.defaultPriceInCents;
    const duration = typeof payload?.durationMinutes === "number"
