import { atTime, toDateKey } from "@/lib/utils/datetime";
import type {
  ID,
  ISODateString,
  PaymentMethod,
...
export function periodOf(at: ISODateString | Date): string {
...
export function recurringTransactionId(chargeId: ID, period: string): ID {
  return `${chargeId}-${period.replace("-", "")}`;
}

/** Meio-dia do dia de vencimento: longe da virada do dia em qualquer fuso. */
export function dueDateFor(period: string, dueDay: number): ISODateString {
  return atTime(`${period}-${String(dueDay).padStart(2, "0")}`, 12);
}

export function validateRecurringCharge(
  input: RecurringChargeInput,
): Result<RecurringChargeInput> {
  const description = input.description?.trim() ?? "";
  if (!input.clientId) return err("Escolha o cadastro que paga a mensalidade.");
  if (!description) return err("Descreva a mensalidade.");
  ...
  if (!isPeriod(input.startPeriod)) return err("Escolha o mês de início.");
...
export function monthlyTransaction(
  ...
  if (charge.status !== "ACTIVE" || period < charge.startPeriod) return null;
  return {
...
export function summarizeMonth(
  ...
  const ofMonth = transactions.filter(
    (item) => item.recurringChargeId && item.period === period && item.status !== "CANCELLED",
  );
  const sum = (items: readonly Transaction[]) => items.reduce((total, item) => total + item.amountInCents, 0);
...
export function chargeHistory(chargeId: ID, transactions: readonly Transaction[]): Transaction[] {
  return transactions
...
export function statusTransitionError(
  from: RecurringCharge["status"],
  to: RecurringCharge["status"],
): string | null {
  if (from === "ENDED") return "Mensalidade encerrada não muda. Crie outra, se voltar a cobrar.";
  if (from === to) return "A mensalidade já está nessa situação.";
  return null;
}

export const RECURRING_STATUS_AUDIT: Record<RecurringCharge["status"], string> = {
 ...
 */
export function currentMonthLaunch(
  charge: RecurringCharge,
  meta: { now: ISODateString; userId: ID | null },
): Transaction | null {
  const period = periodOf(meta.now);
  if (charge.lastLaunchedPeriod && charge.lastLaunchedPeriod >= period) return null;
 ...
 */
export function openRecurringChargeError(
  charges: readonly Pick<RecurringCharge, "clientId" | "status">[],
  clientId: ID,
): string | null {
