...
const RULES = [
  { label: "letra maiúscula", test: (password: string) => /[A-Z]/.test(password) },
  { label: "letra minúscula", test: (password: string) => /[a-z]/.test(password) },
  { label: "número", test: (password: string) => /[0-9]/.test(password) },
  {
    label: "símbolo",
    test: (password: string) => [...password].some((character) => PASSWORD_SPECIAL_CHARACTERS.includes(character)),
  },
] as const;
