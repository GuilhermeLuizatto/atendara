...
function accessLabel(account: AccountAccess, nowMs: number): string {
  ...
  return open ? `Liberado até ${formatDate(account.accessUntil!)}` : "Sem acesso vigente";
}
...
export function AccountsPanel() {
  ...
  return <div className="space-y-6">
    <p className="bg-surface-muted text-muted-foreground rounded-lg p-3 text-sm">
      A conta nasce sem acesso. Quem libera é o webhook do gateway, quando a assinatura é paga, ou uma concessão registrada, com tipo, prazo e motivo. Cada cadastro e alteração entra na trilha da operadora. O cadastro não faz cobrança nem envia mensagens.{mode === "demo" ? " Modo local: estes cadastros existem somente neste navegador." : ""}
    </p>
    ...
    <form onSubmit={submit} className="bg-surface border-border space-y-4 rounded-xl border p-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Profissão liberada">{props => <Select {...props} name="profession">{listProfessions().map(p => <option key={p.id} value={p.id}>{p.label}</option>)}</Select>}</Field>
      </div>
      <fieldset><legend className="text-foreground mb-2 text-sm">Módulos liberados</legend><div className="flex flex-wrap gap-4">{APP_MODULES.map(module => <label key={module} className="text-muted-foreground flex min-h-6 items-center gap-2 text-sm"><input type="checkbox" className="accent-primary size-4" checked={modules.includes(module)} onChange={e => setModules(current => e.target.checked ? [...current, module] : current.filter(m => m !== module))} />{MODULE_LABELS[module]}</label>)}</div></fieldset>
    ...
    <section className="space-y-4" aria-busy={accounts.status === "loading" || undefined}><h2 className="text-foreground font-semibold">Profissionais cadastrados</h2>
      ...
      {professionals.map(account => <article key={account.userId} aria-labelledby={`conta-${account.userId}`} className="bg-surface border-border space-y-4 rounded-xl border p-5">
        ...
        <fieldset><legend className="text-foreground mb-2 text-sm">Permissões</legend><div className="flex flex-wrap gap-4">{APP_MODULES.map(module => <label key={module} className="text-muted-foreground flex min-h-6 items-center gap-2 text-sm"><input type="checkbox" className="accent-primary size-4" checked={account.modules.includes(module)} onChange={e => edit(account.userId, { modules: e.target.checked ? [...account.modules, module] : account.modules.filter(m => m !== module) })} />{MODULE_LABELS[module]}</label>)}</div></fieldset>
