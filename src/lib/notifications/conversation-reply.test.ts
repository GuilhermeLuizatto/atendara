...
/** O portão da resposta da assistente, uma trava por teste (proposta de 24/09). */
...
describe("a resposta da assistente", () => {
  ...
  describe("as travas da regra 11 valem para a resposta", () => {
    ...
    it("sem regra para a resposta, ou regra desligada", () => {
      expect(motivo({ organization: organization({ verifiedSenderChannels: ["WHATSAPP"], rules: [] }) })).toBe(
        "NO_RULE_FOR_EVENT",
      );
      expect(
        motivo({
    ...
    it("remetente não comprovado, não cadastrado ou não aprovado", () => {
      expect(motivo({ organization: organization({ verifiedSenderChannels: [], rules: [regra()] }) })).toBe(
        "SENDER_NOT_VERIFIED",
      );
      expect(motivo({ sender: null })).toBe("SENDER_NOT_REGISTERED");
      expect(motivo({ sender: remetente({ status: "PENDING" }) })).toBe("SENDER_NOT_APPROVED");
    });

    it("em modo de teste, destino fora da lista de testadores", () => {
      expect(motivo({ sender: remetente({ testRecipients: ["+5500911111111"] }) })).toBe(
        "DESTINATION_NOT_IN_TEST_LIST",
      );
    });

    it("sem consentimento, com consentimento só de outro canal, ou retirado", () => {
      expect(motivo({ client: client({ appointmentNotificationsEnabled: false }) })).toBe("MISSING_CONSENT");
      expect(motivo({ client: client({ notificationConsent: consent({ EMAIL: [consentRecord()] }) }) })).toBe(
    ...
    it("sem contato válido", () => {
      expect(motivo({ client: client({ phone: null }) })).toBe("MISSING_CONTACT");
    });
  });

  describe("o que só a conversa sabe", () => {
    it("número sem cadastro único não recebe resposta", () => {
      expect(motivo({ client: null })).toBe("CLIENT_NOT_IDENTIFIED");
    });

    it("conversa com a equipe ou crítica não recebe automação", () => {
      expect(
        motivo({ conversation: { escalated: true, attention: "HIGH", inboundWindowEndsAt: DEPOIS_DA_MENSAGEM } }),
    ...
    it("fora das 24 horas que a pessoa abriu, ou sem janela registrada", () => {
      expect(motivo({ now: "2026-09-11T12:00:01.000Z", validUntil: null })).toBe("REPLY_WINDOW_CLOSED");
      expect(
        motivo({ conversation: { escalated: false, attention: "NORMAL", inboundWindowEndsAt: undefined } }),
      ).toBe("REPLY_WINDOW_CLOSED");
    });

    it("oferta com a reserva vencida não sai", () => {
      expect(motivo({ now: "2026-09-10T12:10:01.000Z" })).toBe("REPLY_EXPIRED");
    });

    it("texto que não se completa não sai", () => {
      expect(motivo({ details: { slots: [] } })).toBe("TEMPLATE_REJECTED");
    });
  });
});
