...
export function PasswordSetup() {
  ...
  return <main className="bg-background flex min-h-dvh items-center justify-center p-6">
    <form onSubmit={submit} className="border-border bg-surface w-full max-w-md space-y-5 rounded-xl border p-6">
      ...
      <Field label="Nova senha" hint={PASSWORD_HINT}>{props => <Input {...props} type="password" autoComplete="new-password" minLength={12} maxLength={128} required value={password} onChange={e => setPassword(e.target.value)} />}</Field>
