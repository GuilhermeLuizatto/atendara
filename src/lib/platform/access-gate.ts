...
export function accessGrantReasonError(reason: string): string | null {
  ...
  if (length < ACCESS_GRANT_REASON_LENGTH.min) return "Explique o motivo da concessao.";
  if (length > ACCESS_GRANT_REASON_LENGTH.max) return "Resuma o motivo da concessao.";
  return null;
}
