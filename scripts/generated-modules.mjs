...
export const SOURCES = [
  ...
  ["src/lib/auth/password-policy.ts", "password-policy"],
  ["src/lib/platform/trial.ts", "trial"],
  ...
  ["src/config/services.ts", "services-config"],
  ["src/lib/agenda/services.ts", "agenda-services"],
  ...
  ["src/lib/finance/receipts.ts", "finance-receipts"],
  ...
  [
    "src/lib/notifications/providers/simulated.ts",
    "notifications-providers-simulated",
  ],
  [
    "src/lib/notifications/providers/n8n-bridge.ts",
    "notifications-providers-n8n-bridge",
  ],
  ...
  [
    "src/lib/automation/appointment-changes.ts",
    "automation-appointment-changes",
  ],
  ...
  ["src/config/classifications.ts", "classifications-config"],
  ["src/lib/rules/evaluate.ts", "rules-evaluate"],
  ["src/lib/rules/context.ts", "rules-context"],
  ["src/lib/rules/precedence.ts", "rules-precedence"],
  ["src/lib/rules/natural-language.ts", "rules-natural-language"],
  ["src/config/inbound.ts", "inbound-config"],
  ...
  [
    "src/lib/agenda/calendar-connection-alert.ts",
    "agenda-calendar-connection-alert",
  ],
...
/**
 * Gera em memoria todos os modulos que as functions leem.
 *
 * Separado da escrita de proposito: assim o teste compara o que o gerador
 * produz com o que esta commitado, sem tocar em `functions/generated/`.
 */
export function generateModules() {
  ...
  for (const [source, target] of SOURCES) {
    ...
    modules.set(target, "// Gerado por scripts/build-functions.mjs.\n" + code);
  }
