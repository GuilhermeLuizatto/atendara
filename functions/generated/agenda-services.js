...
import { SERVICE_ERRORS, SERVICE_LIMITS } from "./services-config.js";
function limpar(texto) {
    const valor = texto?.trim() ?? "";
    return valor.length > 0 ? valor : null;
}
export function validateService(input, context) {
    const name = input.name.trim();
    if (name.length < SERVICE_LIMITS.name.min)
        return { ok: false, error: SERVICE_ERRORS.NAME_REQUIRED };
    if (name.length > SERVICE_LIMITS.name.max)
        return { ok: false, error: SERVICE_ERRORS.NAME_TOO_LONG };
    ...
    const repetido = context.existing.some((servico) => servico.id !== context.editingId && servico.name.trim().toLowerCase() === name.toLowerCase());
    if (repetido)
        return { ok: false, error: SERVICE_ERRORS.DUPLICATE_NAME };
...
export function bookableServices(services) {
    return services
 ...
 */
export function appointmentDefaultsFor(service) {
    if (!service)
        return { serviceId: null, serviceName: null, durationMinutes: null, priceInCents: null };
    return {
...
export function nextServicePosition(services) {
